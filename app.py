...
if run:
    ...
    with evaluator_ph:
        ...
        if result.matches:
            ...
            df = pd.DataFrame(
                [{"Trial": (m.title[:40] + "…") if len(m.title) > 40 else m.title, "Score": m.match_score, "NCT ID": m.nct_id} for m in result.matches]
            )
            chart = (
                alt.Chart(df)
                ...
                .encode(
                    ...
                    y=alt.Y("Trial:N", sort="-x", title=None),
                    color=alt.Color(
                        "Score:Q",
                        scale=alt.Scale(domain=[0, 40, 70, 100], range=["#F2707A", "#F2707A", "#F2B155", "#2DD4BF"]),
                        legend=None,
                    ),
                    tooltip=["Trial", "Score", "NCT ID"],
                )
                .properties(height=max(120, 32 * len(df)))
                .configure_axis(labelColor="#8C97AC", titleColor="#8C97AC", grid=False)
